package com.yaskulsky.equivox.network.commands.client;
...
import com.yaskulsky.equivox.emc.mappers.OreBlacklistMapper;
import com.yaskulsky.equivox.emc.mappers.RawMaterialsBlacklistMapper;
...
public class DumpMissingEmc {
	...
	private static final boolean SKIP_TOP = Boolean.parseBoolean(System.getProperties().getProperty("equivox.skip_top"));
	...
	private static boolean expectedMissing(FeatureFlagSet enabledFeatures, ItemInfo info) {
		...
		if (switch (holder.value()) {
			...
			case Tome tome when !EquivoxConfig.common.craftableTome.get() -> true;
			default -> false;
		}) {
			return true;
